...
static void timer_sample_start(uint32_t delay_ms)
{
	...
	if (err < 0) {
		LOG_ERR("k_work_reschedule timer_sample_work, error: %d", err);
		SEND_FATAL_ERROR();
...
static void timer_sample_stop(void)
{
	...
	if (err < 0) {
		LOG_ERR("k_work_cancel_delayable timer_sample_work, error: %d", err);
	}
}

static void send_battery_percentage_sample_response(struct power_state_object *state_object)
{
	int err;
...
static int sample_and_process(struct power_state_object *state_object)
{
	...
	err = charger_read_sensors(state_object->charger, &state_object->voltage,
				   &state_object->current, &state_object->temperature,
	...
	state_object->percentage = nrf_fuel_gauge_process(state_object->voltage,
							 state_object->current, state_object->temperature,
							 delta, NULL);
...
static void lte_lc_evt_handler(const struct lte_lc_evt *const evt)
{
	...
	case LTE_LC_EVT_MODEM_SLEEP_EXIT:
		LOG_DBG("Modem sleep exit event received");
	...
	case LTE_LC_EVT_MODEM_SLEEP_ENTER:
		LOG_DBG("Modem sleep enter event received");
...
static enum smf_state_result state_idle_run(void *obj)
{
	...
	if (state_object->chan == &priv_power_chan) {
		...
		if (msg->type == POWER_PRIV_MODEM_SLEEP_EXIT) {
			LOG_DBG("Modem sleep exit detected, transitioning to active state");
		...
		if (msg->type == POWER_PRIV_MODEM_SLEEP_ENTRY) {
			LOG_DBG("Modem sleep entry detected, already in idle state, ignoring");
			return SMF_EVENT_HANDLED;
		}
	}
...
static enum smf_state_result state_active_run(void *obj)
{
	...
	if (state_object->chan == &priv_power_chan) {
		...
		if (msg->type == POWER_PRIV_TIMER_EXPIRED) {
			LOG_DBG("Sample timer expired");
			sample_and_process(state_object);
			return SMF_EVENT_HANDLED;
		}

		if (msg->type == POWER_PRIV_MODEM_SLEEP_ENTRY) {
			LOG_DBG("Modem sleep entry detected, transitioning to idle state");
			smf_set_state(SMF_CTX(state_object), &states[STATE_IDLE]);
			return SMF_EVENT_HANDLED;
		}

		if (msg->type == POWER_PRIV_MODEM_SLEEP_EXIT) {
			LOG_DBG("Modem sleep exit detected, already in active state, ignoring");
			return SMF_EVENT_HANDLED;
		}
	}
...
static void power_module_thread(void)
{
	...
	err = charger_read_sensors(power_state.charger, &parameters.v0, &parameters.i0, &parameters.t0,
				   &chg_status, NULL);
	if (err)
	{
	...
	/* Seed the initial SoC so that a sample request arriving before the first
	 * periodic timer fires does not return 0.0%. delta=0 means no state
	 * advancement, it just obtains the estimate from the freshly-initialised
	 * fuel gauge model.
	 */
	power_state.voltage = parameters.v0;
	power_state.current = parameters.i0;
	power_state.temperature = parameters.t0;
	power_state.percentage = nrf_fuel_gauge_process(parameters.v0, parameters.i0,
						    parameters.t0, 0.0f, NULL);
	...
	while (true) {
		...
		err = zbus_sub_wait_msg(&power, &power_state.chan,
					 power_state.msg_buf, zbus_wait_ms);
		if (err == -ENOMSG) {
			continue;
		} else if (err) {
